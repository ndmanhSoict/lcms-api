...
export class StudentController {
  ...
  createStudent = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      sendCreated(res, result, 'Đã tạo hồ sơ học sinh và tài khoản phụ huynh thành công');
    } catch (error) { next(error); }
  };

  addParent = async (req: Request<{ studentId: string }>, res: Response, next: NextFunction) => {
    try {
      const result = await this.studentService.addSecondParent(req.params.studentId, req.body);
  ...
  getStudents = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      sendPaginated(res, result.students, meta as any, 'Lấy danh sách học sinh thành công');
    } catch (error) { next(error); }
  };

  getStudentById = async (req: Request<{ id: string }>, res: Response, next: NextFunction) => {
    try {
      const result = await this.studentService.getStudentById(req.params.id);
