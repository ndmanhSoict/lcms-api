...
export class GradeRecordController {
  ...
  getStudentGradeRecords = async (
    req: Request<{ studentId: string }>,
    res: Response,
    next: NextFunction
  ) => {
    try {
      const result = await this.service.getStudentGradeRecords(
  ...
  getClassGradeRecords = async (
    req: Request<{ classId: string }>,
    res: Response,
    next: NextFunction
  ) => {
    try {
      const result = await this.service.getClassGradeRecords(
        req.params.classId,
        req.query,
        req.user
      );
      sendSuccess(res, result, 'Lấy học bạ lớp học thành công');
    } catch (error) {
      next(error);
    }
  };
