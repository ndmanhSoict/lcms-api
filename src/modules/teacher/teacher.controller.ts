...
export class TeacherController {
  ...
  getTeachers = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      sendPaginated(res, result.teachers, meta, 'Lấy danh sách giáo viên thành công');
    } catch (error) {
      next(error);
    }
  };
  ...
  calculateSalary = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this.service.calculateSalary(req.query, req.user);
      sendSuccess(res, result, 'Tính lương giáo viên thành công');
    } catch (error) {
      next(error);
    }
  };

  getSalaryOverview = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this.service.getSalaryOverview(req.query, req.user);
      sendSuccess(res, result, 'Lấy tổng quan kỳ lương giáo viên thành công');
    } catch (error) {
      next(error);
    }
  };
