...
export class ClassAnnouncementService {
  ...
  async createAnnouncement(classId: string, body: any, requester: any) {
    const cls = await this.repo.findClassById(classId);
    if (!cls) throw new NotFoundError('Lớp học');
    ...
    const announcement = await this.repo.create({
      ...
      targetAudience: body.target_audience ?? ['student', 'parent'],
    ...
    console.log(
      `[Notification] Thông báo mới "${body.title}" cho lớp ${classId} — đối tượng: ${(body.target_audience ?? []).join(', ')}`
    );
  ...
  async getAnnouncements(classId: string, query: any, requester: any) {
    await this.checkClassAccess(classId, requester);

    const { page, limit, skip } = getPagination(query.page, query.limit);
    const { items, total } = await this.repo.findByClass(classId, skip, limit);
