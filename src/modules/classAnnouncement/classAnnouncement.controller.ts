...
export class ClassAnnouncementController {
  ...
  createAnnouncement = async (req: Request<{ classId: string }>, res: Response, next: NextFunction) => {
    try {
  ...
  getAnnouncements = async (req: Request<{ classId: string }>, res: Response, next: NextFunction) => {
    try {
      const { data, meta } = await this.service.getAnnouncements(req.params.classId, req.query, req.user);
      sendSuccess(res, data, 'Lấy danh sách thông báo lớp thành công', 200, meta as unknown as Record<string, unknown>);
    } catch (error) { next(error); }
  };
}
