...
export class EnrollmentService {
  ...
  async enrollStudent(data: { studentId: string; classId: string }, requester: RequestUser) {
    ...
    const cls = await Class.findById(classId).populate('teacherId', 'fullName branchId').lean();
    ...
    const populatedTeacher = cls.teacherId as PopulatedUserSummary | undefined;
    if (
      populatedTeacher?.branchId &&
      populatedTeacher.branchId.toString() !== cls.branchId.toString()
    ) {
      throw new ForbiddenError('Giáo viên của lớp không thuộc cơ sở của lớp học');
    }
  ...
  async leaveClass(
    enrollmentId: string,
    leaveData: { reason: string; note?: string },
    ...
    if (
      ...
      enrollment.branchId?.toString() !== requester.branchId
    ) {
    ...
    await runOptionalTransaction(async session => {
      ...
      await this.enrollmentRepo.removeClassFromStudent(
  ...
  async getStudentEnrollments(studentId: string, requester: RequestUser) {
    ...
    return await this.enrollmentRepo.getEnrollmentsByStudent(
      studentId,
