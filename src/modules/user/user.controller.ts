...
export class UserController {
  ...
  getUsers = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      sendPaginated(res, result.users, meta, 'Lấy danh sách người dùng thành công');
    } catch (error) {
      next(error);
    }
  };
  ...
  getMe = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this.userService.getUserById(req.user.id, req.user);
      sendSuccess(res, result, 'Lấy hồ sơ cá nhân thành công');
    } catch (error) {
      next(error);
    }
  };
  ...
  updateMe = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this.userService.updateUser(req.user.id, req.body, req.user);
      sendSuccess(res, result, 'Cập nhật hồ sơ cá nhân thành công');
    } catch (error) {
      next(error);
    }
  };
  ...
  updateUserStatus = async (
    ...
    try {
      ...
      sendSuccess(
        res,
        { isActive: result?.isActive ?? req.body.isActive },
        req.body.isActive ? 'Kích hoạt tài khoản thành công' : 'Tạm ngừng tài khoản thành công'
      );
    } catch (error) {
      next(error);
    }
